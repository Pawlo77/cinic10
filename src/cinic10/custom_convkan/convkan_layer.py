...
class ConvKAN(torch.nn.Module):
    def __init__(
        ...
        self._in_dim = (in_channels // groups) * self.kernel_size[0] * self.kernel_size[1]
    ...
    def forward(self, x):
        ...
        # Ensure contiguous memory layout before passing to subsequent ops
        # (BatchNorm on MPS may call view internally which requires contiguity).
