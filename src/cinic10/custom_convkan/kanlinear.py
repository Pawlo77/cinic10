# The implementation of the KANLinear layer is taken from https://github.com/Blealtan/efficient-kan
# with modification that enables grouped convolution provided by https://github.com/minh-nguyenhoang
...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        self.base_weight = torch.nn.Parameter(torch.empty(groups, out_features, in_features))
        self.spline_weight = torch.nn.Parameter(
            torch.empty(groups, out_features, in_features, grid_size + spline_order)
        )
        if enable_standalone_scale_spline:
            self.spline_scaler = torch.nn.Parameter(torch.empty(groups, out_features, in_features))
    ...
    def reset_parameters(self):
        ...
        with torch.no_grad():
            ...
            self.spline_weight.data.copy_(
                ...
                * self.curve2coeff(
                    self.grid.transpose(-1, -2)[:, self.spline_order : -self.spline_order],
                    noise,
                )
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        base_output = torch.bmm(self.base_activation(x), self.base_weight.permute(0, 2, 1))
        spline_output = torch.bmm(
            self.b_splines(x).reshape(x.size(0), x.size(1), -1),
            self.scaled_spline_weight.reshape(x.size(0), self.out_features, -1).permute(0, 2, 1),
        )
    ...
    def update_grid(self, x: torch.Tensor, margin=0.01):
        ...
        unreduced_spline_output = torch.matmul(splines, orig_coeff)  # (groups, in, batch, out)
        ...
        grid = torch.cat(
            [
                grid[:, :1]
                - uniform_step.unsqueeze(1)
                * torch.arange(self.spline_order, 0, -1, device=x.device).unsqueeze(1).unsqueeze(0),
                grid,
                grid[:, -1:]
                + uniform_step.unsqueeze(1)
                * torch.arange(1, self.spline_order + 1, device=x.device).unsqueeze(1).unsqueeze(0),
            ],
            dim=1,
        )
